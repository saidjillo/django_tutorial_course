...
def game_info(request, game_id):
    ...
    if request.method == 'GET':
        ...
        if user.groups.filter(name='developers').count() !=0:
            return redirect('shop:index')
        game        = get_object_or_404(Game, pk=game_id)
        secret_key  = 'f8844b48591841836b4e42d26fcf755c'
        pid         = 'coastseller'
        sid         = 'myshopapp'
        amount      =  game.price
        ...
        if transaction.count() !=0:
            return render(request, "shop/index.html", {"error": "Game is already in catalog"})
        else:
            return render(request, 'shop/game_info.html', {'game':game,'url':url,'pid':pid,
                                                            'sid':sid,'amount':amount,'success':success,
...
def search(request):
    if request.method == 'POST':
        ...
        if not query:
            return render(request, 'shop/search_result.html', {'error':'0 items returned'})
...
def create_game(request):
    if request.method == 'POST':
        ...
        if not title and not url and not price:
            return render(request, "shop/publish_game_form.html", {"error": "Please fill in all the required fields"})
        ...
        if float_price <= 0:
            return render(request, "shop/publish_game_form.html", {"error": "Price cannot be zero"})
            
        # validate url
        try:
            URLValidator()(url)
        except ValidationError:
            return render(request, "shop/publish_game_form.html", {"error": "Invalid url"})

        try:
            Game.objects.create(title=title, price=price, url=url, developer=developer)
    ...
    if request.method == 'POST':
        ...
        if game.developer.user_id == user.id:
            ...
            if not title and not price and not url:
                return render(request, 'shop/edit_game.html', {'error':'All fields are required',
                                                               'game':game})
            ...
            if price.strip():
                ...
                if float_price <=0:
                    return render(request, "shop/publish_game_form.html", {'error':'Price cannot be 0',
                                                               'game':game})
